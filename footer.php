...
?>
<script>
    document.querySelector('.tsLink a').addEventListener('click',function(){
        $("#tech-support").fadeIn('slow');
    });

        $("#tech-support-go-back").click( function(){
            $("#tech-support").fadeOut('slow');
            console.log('hiding the tech support.');
        });

        $("#on-phone").click( function(){
            $("#tech-support").fadeOut('slow');
            $("#download-lmi-rescue-link").show();
        });

        $("#download-go-back").click( function(){
            $("#download-lmi-rescue-link").hide();
        });

        $("#not-on-phone").click( function(){
            $("#tech-support").fadeOut('slow');
            $("#tech-support-contact-form").show();
            $(".your-name input").focus();
        });

        $("#cancel-contact-form").click (function(){
            $("#tech-support").fadeOut('slow');
            $("#tech-support-contact-form").hide();
        });

//  $("#slide-out-widget-area .menuopen li:nth-child(6) a").on("click", function(e){
//       e.preventDefault();
//      $("#tech-support").css('display','block');
//    });
</script>
