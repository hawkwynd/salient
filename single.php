<?php
/**
 * single.php handles all standard POSTS, BLOGS etc.
 * Not special post-types.
 */
...
<div class="container-wrap <?php echo ($fullscreen_header == true) ? 'fullscreen-blog-header': null; ?> <?php if($blog_type == 'std-blog-fullwidth' || $hide_sidebar == '1') echo 'no-sidebar'; ?>">
	...
	<div class="container main-content">
        ...
        ?>
        <?php include('tech-support-section.php'); ?>
        ...
        /* if the FEU->Get_Field_Value 1st character of the value == "Y" for the post to edit, then display the button for edit */
            echo  substr( strtoupper($FEUP->Get_Field_Value( $post->post_title) ), 0,1)  == "Y" ? '<div style="margin-bottom: 14px;"><a style="padding: 15px 20px; border-radius: 25px; color: white; background-color: #263975;" href="'. get_permalink($post->ID) .'?action=edit">Edit '. $post->post_title . '</a></div>' : '';
			    ...
			    if((empty($bg) && empty($bg_color)) && $fullscreen_header != true) { ?>
					...
					<div class="row heading-title" data-header-style="<?php echo $blog_header_type; ?>">
                        ...
                        <?php } ?>
                            ...
                            <h3 class="entry-title" rel="single.php"><?php the_title(); ?></h3>
							
							<div id="single-below-header" rel="scott">
